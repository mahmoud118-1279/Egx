...
if st.sidebar.button("🧹 تنظيف البيانات القديمة (90 يوم)"):
    with st.spinner("جاري تنظيف البيانات القديمة..."):
        analyst.clear_old_predictions(days=90)
        st.sidebar.success("✅ تم تنظيف البيانات القديمة بنجاح!")

# ✅ زر تصدير بيانات التعلم
if st.sidebar.button("📤 تصدير بيانات التعلم"):
    with st.spinner("جاري تصدير البيانات..."):
        file_path = analyst.export_learning_data()
        if file_path:
            st.sidebar.success(f"✅ تم التصدير إلى {file_path}")
        else:
            st.sidebar.error("❌ فشل في التصدير")
                ...
                try:
                    ...
                    try:
                        ...
                        analyst.record_prediction(
                            ...
                            direction=dir_out,
                            current_indicators=indicators_dict  # ✅ dict
                        )
                    except Exception as e:
                        print(f"⚠️ خطأ في تسجيل التحليل لـ {row['symbol']}: {e}")
            ...
            with st.spinner("جاري جمع البيانات وحساب المؤشرات..."):
                ...
                if not s_df.empty and len(s_df) >= 30:
                    ...
                    try:
                        ...
                        st.warning(f"⚠️ خطأ في تسجيل التحليل: {e}")

                    st.success(f"✅ تم تخزين تحليل {row_choice['symbol']} في ملف التعلم الذاتي")

                    # ✅ إرسال إشعار تليجرام إذا كانت إشارة شراء
                    if "شراء" in dir_out:
                        alert_msg = (
